...
export async function POST(request: NextRequest) {
  try {
    ...
    const ref = await adminDb.collection("events").add({ name: String(body.name).trim().slice(0, 160), slug, description: String(body.description || "").slice(0, 2000), venue: String(body.venue || "").slice(0, 300), bannerUrl: String(body.bannerUrl || "").slice(0, 500), accessMode: body.accessMode || "code", joinUrl: String(body.joinUrl || "").slice(0, 500), joinInstructions: String(body.joinInstructions || "").slice(0, 1000), startAt: body.startAt || null, endAt: body.endAt || null, timezone: body.timezone || "Africa/Lagos", status: "draft" as EventStatus, form: { title: String(body.form.title).slice(0, 160), introText: String(body.form.introText || "").slice(0, 1000), successMessage: String(body.form.successMessage || "Registration received.").slice(0, 500), fields }, createdAt: now, updatedAt: now, createdBy: actor.uid });
